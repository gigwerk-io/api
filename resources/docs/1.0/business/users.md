# Users

This allows a business owner to manage the users of their marketplace.

---

- [All Users](#all-users)


- [Show User](#show-user)


- [Update Role](#update-role)


- [Remove User](#remove-user)



<a name="all-users"></a>
## All Users

View all users that are apart of a business marketplace.
### Endpoint
|Method|URI|Authentication|
|:-|:-|:-|
|`GET`|`/business/{unique_id}/users`|`true`|



> {success} Example Success Response
Code `200`

Content

```json
{
    "success": true,
    "message": "Show all users",
    "data": [
        {
            "id": 1,
            "first_name": "Peter",
            "last_name": "Weyland",
            "username": "admin_one",
            "email": "admin_one@mail.com",
            "phone": "(460) 419-8167",
            "apn_token": null,
            "fcm_token": null,
            "email_verified_at": "2020-09-22T13:23:49.000000Z",
            "last_seen_at": null,
            "deleted_at": null,
            "created_at": "2020-09-22T13:23:49.000000Z",
            "updated_at": "2020-09-22T13:23:49.000000Z",
            "role": "Customer",
            "rating": null,
            "isActive": false,
            "lastSeen": null,
            "pivot": {
                "business_id": 1,
                "user_id": 1,
                "role_id": 3,
                "apn_token": null,
                "fcm_token": null,
                "email_notifications": true,
                "sms_notifications": true,
                "push_notifications": true,
                "role": {
                    "id": 3,
                    "name": "Customer"
                }
            },
            "profile": {
                "id": 1,
                "user_id": 1,
                "image": "https:\/\/gigwerk-disk.s3.amazonaws.com\/seed\/peter-weyland.png",
                "description": "Founder and owner of the Weyland Corporation",
                "created_at": "2020-09-22T13:23:49.000000Z",
                "updated_at": "2020-09-22T13:23:49.000000Z"
            }
        },
        {
            "id": 2,
            "first_name": "Angelina",
            "last_name": "Jennie",
            "username": "worker_one",
            "email": "worker_one@mail.com",
            "phone": "589-200-0246",
            "apn_token": null,
            "fcm_token": null,
            "email_verified_at": "2020-09-22T13:23:54.000000Z",
            "last_seen_at": null,
            "deleted_at": null,
            "created_at": "2020-09-22T13:23:54.000000Z",
            "updated_at": "2020-09-22T13:23:54.000000Z",
            "role": "Verified Freelancer",
            "rating": 5,
            "isActive": false,
            "lastSeen": null,
            "pivot": {
                "business_id": 1,
                "user_id": 2,
                "role_id": 1,
                "apn_token": null,
                "fcm_token": null,
                "email_notifications": true,
                "sms_notifications": true,
                "push_notifications": true,
                "role": {
                    "id": 1,
                    "name": "Verified Freelancer"
                }
            },
            "profile": {
                "id": 2,
                "user_id": 2,
                "image": "https:\/\/randomuser.me\/api\/portraits\/men\/92.jpg",
                "description": "Molestiae consequuntur dolores omnis possimus.",
                "created_at": "2020-09-22T13:23:54.000000Z",
                "updated_at": "2020-09-22T13:23:54.000000Z"
            }
        },
        {
            "id": 3,
            "first_name": "Marie",
            "last_name": "Chaya",
            "username": "pending_one",
            "email": "pending_one@mail.com",
            "phone": "(732) 273-4002 x9221",
            "apn_token": null,
            "fcm_token": null,
            "email_verified_at": "2020-09-22T13:23:54.000000Z",
            "last_seen_at": null,
            "deleted_at": null,
            "created_at": "2020-09-22T13:23:54.000000Z",
            "updated_at": "2020-09-22T13:23:54.000000Z",
            "role": "Pending Freelancer",
            "rating": null,
            "isActive": false,
            "lastSeen": null,
            "pivot": {
                "business_id": 1,
                "user_id": 3,
                "role_id": 2,
                "apn_token": null,
                "fcm_token": null,
                "email_notifications": true,
                "sms_notifications": true,
                "push_notifications": true,
                "role": {
                    "id": 2,
                    "name": "Pending Freelancer"
                }
            },
            "profile": {
                "id": 3,
                "user_id": 3,
                "image": "https:\/\/randomuser.me\/api\/portraits\/men\/94.jpg",
                "description": "Est perspiciatis nisi iure natus.",
                "created_at": "2020-09-22T13:23:54.000000Z",
                "updated_at": "2020-09-22T13:23:54.000000Z"
            }
        },
        {
            "id": 9,
            "first_name": "Melody",
            "last_name": "Estell",
            "username": "multi_user",
            "email": "grant.maximillian@example.org",
            "phone": "1-213-789-9004",
            "apn_token": null,
            "fcm_token": null,
            "email_verified_at": "2020-09-22T13:23:59.000000Z",
            "last_seen_at": null,
            "deleted_at": null,
            "created_at": "2020-09-22T13:23:59.000000Z",
            "updated_at": "2020-09-22T13:23:59.000000Z",
            "role": "Verified Freelancer",
            "rating": null,
            "isActive": false,
            "lastSeen": null,
            "pivot": {
                "business_id": 1,
                "user_id": 9,
                "role_id": 1,
                "apn_token": null,
                "fcm_token": null,
                "email_notifications": true,
                "sms_notifications": true,
                "push_notifications": true,
                "role": {
                    "id": 1,
                    "name": "Verified Freelancer"
                }
            },
            "profile": {
                "id": 9,
                "user_id": 9,
                "image": "https:\/\/randomuser.me\/api\/portraits\/men\/96.jpg",
                "description": "Sit debitis incidunt optio dolores consequatur.",
                "created_at": "2020-09-22T13:23:59.000000Z",
                "updated_at": "2020-09-22T13:23:59.000000Z"
            }
        },
        {
            "id": 10,
            "first_name": "Maiya",
            "last_name": "Anabel",
            "username": "dario.wuckert51",
            "email": "collin31@example.net",
            "phone": "204.332.9534",
            "apn_token": null,
            "fcm_token": null,
            "email_verified_at": "2020-09-22T13:23:59.000000Z",
            "last_seen_at": null,
            "deleted_at": null,
            "created_at": "2020-09-22T13:23:59.000000Z",
            "updated_at": "2020-09-22T13:23:59.000000Z",
            "role": "Verified Freelancer",
            "rating": null,
            "isActive": false,
            "lastSeen": null,
            "pivot": {
                "business_id": 1,
                "user_id": 10,
                "role_id": 1,
                "apn_token": null,
                "fcm_token": null,
                "email_notifications": true,
                "sms_notifications": true,
                "push_notifications": true,
                "role": {
                    "id": 1,
                    "name": "Verified Freelancer"
                }
            },
            "profile": {
                "id": 10,
                "user_id": 10,
                "image": "https:\/\/randomuser.me\/api\/portraits\/men\/91.jpg",
                "description": "Odit quod et consequatur veritatis aut et nihil.",
                "created_at": "2020-09-22T13:23:59.000000Z",
                "updated_at": "2020-09-22T13:23:59.000000Z"
            }
        },
        {
            "id": 11,
            "first_name": "Tomasa",
            "last_name": "Conrad",
            "username": "thora7425",
            "email": "schultz.litzy@example.com",
            "phone": "+1-327-645-4325",
            "apn_token": null,
            "fcm_token": null,
            "email_verified_at": "2020-09-22T13:23:59.000000Z",
            "last_seen_at": null,
            "deleted_at": null,
            "created_at": "2020-09-22T13:23:59.000000Z",
            "updated_at": "2020-09-22T13:23:59.000000Z",
            "role": "Verified Freelancer",
            "rating": null,
            "isActive": false,
            "lastSeen": null,
            "pivot": {
                "business_id": 1,
                "user_id": 11,
                "role_id": 1,
                "apn_token": null,
                "fcm_token": null,
                "email_notifications": true,
                "sms_notifications": true,
                "push_notifications": true,
                "role": {
                    "id": 1,
                    "name": "Verified Freelancer"
                }
            },
            "profile": {
                "id": 11,
                "user_id": 11,
                "image": "https:\/\/randomuser.me\/api\/portraits\/men\/97.jpg",
                "description": "Doloribus id minima fuga consequatur illum atque voluptate.",
                "created_at": "2020-09-22T13:23:59.000000Z",
                "updated_at": "2020-09-22T13:23:59.000000Z"
            }
        },
        {
            "id": 12,
            "first_name": "Emmanuelle",
            "last_name": "Zelma",
            "username": "gerhold.bridie10",
            "email": "geovanny59@example.com",
            "phone": "548-455-2241",
            "apn_token": null,
            "fcm_token": null,
            "email_verified_at": "2020-09-22T13:23:59.000000Z",
            "last_seen_at": null,
            "deleted_at": null,
            "created_at": "2020-09-22T13:23:59.000000Z",
            "updated_at": "2020-09-22T13:23:59.000000Z",
            "role": "Verified Freelancer",
            "rating": null,
            "isActive": false,
            "lastSeen": null,
            "pivot": {
                "business_id": 1,
                "user_id": 12,
                "role_id": 1,
                "apn_token": null,
                "fcm_token": null,
                "email_notifications": true,
                "sms_notifications": true,
                "push_notifications": true,
                "role": {
                    "id": 1,
                    "name": "Verified Freelancer"
                }
            },
            "profile": {
                "id": 12,
                "user_id": 12,
                "image": "https:\/\/randomuser.me\/api\/portraits\/men\/91.jpg",
                "description": "Eveniet aut sint eaque.",
                "created_at": "2020-09-22T13:23:59.000000Z",
                "updated_at": "2020-09-22T13:23:59.000000Z"
            }
        },
        {
            "id": 13,
            "first_name": "Ephraim",
            "last_name": "Frieda",
            "username": "sage.collins70",
            "email": "trycia.denesik@example.com",
            "phone": "+1-483-755-6664",
            "apn_token": null,
            "fcm_token": null,
            "email_verified_at": "2020-09-22T13:23:59.000000Z",
            "last_seen_at": null,
            "deleted_at": null,
            "created_at": "2020-09-22T13:23:59.000000Z",
            "updated_at": "2020-09-22T13:23:59.000000Z",
            "role": "Verified Freelancer",
            "rating": null,
            "isActive": false,
            "lastSeen": null,
            "pivot": {
                "business_id": 1,
                "user_id": 13,
                "role_id": 1,
                "apn_token": null,
                "fcm_token": null,
                "email_notifications": true,
                "sms_notifications": true,
                "push_notifications": true,
                "role": {
                    "id": 1,
                    "name": "Verified Freelancer"
                }
            },
            "profile": {
                "id": 13,
                "user_id": 13,
                "image": "https:\/\/randomuser.me\/api\/portraits\/women\/91.jpg",
                "description": "Perferendis eius consequuntur quis id et.",
                "created_at": "2020-09-22T13:23:59.000000Z",
                "updated_at": "2020-09-22T13:23:59.000000Z"
            }
        },
        {
            "id": 14,
            "first_name": "Kaylie",
            "last_name": "Christop",
            "username": "emmett6166",
            "email": "ole28@example.com",
            "phone": "1-467-689-2938 x7278",
            "apn_token": null,
            "fcm_token": null,
            "email_verified_at": "2020-09-22T13:23:59.000000Z",
            "last_seen_at": null,
            "deleted_at": null,
            "created_at": "2020-09-22T13:23:59.000000Z",
            "updated_at": "2020-09-22T13:23:59.000000Z",
            "role": "Verified Freelancer",
            "rating": null,
            "isActive": false,
            "lastSeen": null,
            "pivot": {
                "business_id": 1,
                "user_id": 14,
                "role_id": 1,
                "apn_token": null,
                "fcm_token": null,
                "email_notifications": true,
                "sms_notifications": true,
                "push_notifications": true,
                "role": {
                    "id": 1,
                    "name": "Verified Freelancer"
                }
            },
            "profile": {
                "id": 14,
                "user_id": 14,
                "image": "https:\/\/randomuser.me\/api\/portraits\/women\/98.jpg",
                "description": "Nobis similique animi laudantium quos voluptatum.",
                "created_at": "2020-09-22T13:23:59.000000Z",
                "updated_at": "2020-09-22T13:23:59.000000Z"
            }
        },
        {
            "id": 15,
            "first_name": "Brittany",
            "last_name": "Felicita",
            "username": "shany233",
            "email": "vheller@example.net",
            "phone": "+1-897-203-2607",
            "apn_token": null,
            "fcm_token": null,
            "email_verified_at": "2020-09-22T13:23:59.000000Z",
            "last_seen_at": null,
            "deleted_at": null,
            "created_at": "2020-09-22T13:23:59.000000Z",
            "updated_at": "2020-09-22T13:23:59.000000Z",
            "role": "Verified Freelancer",
            "rating": null,
            "isActive": false,
            "lastSeen": null,
            "pivot": {
                "business_id": 1,
                "user_id": 15,
                "role_id": 1,
                "apn_token": null,
                "fcm_token": null,
                "email_notifications": true,
                "sms_notifications": true,
                "push_notifications": true,
                "role": {
                    "id": 1,
                    "name": "Verified Freelancer"
                }
            },
            "profile": {
                "id": 15,
                "user_id": 15,
                "image": "https:\/\/randomuser.me\/api\/portraits\/women\/97.jpg",
                "description": "Impedit voluptatem accusantium ex numquam.",
                "created_at": "2020-09-22T13:23:59.000000Z",
                "updated_at": "2020-09-22T13:23:59.000000Z"
            }
        },
        {
            "id": 16,
            "first_name": "Alfred",
            "last_name": "Larry",
            "username": "cassandra.bosco99",
            "email": "rlabadie@example.com",
            "phone": "+1 (427) 445-4006",
            "apn_token": null,
            "fcm_token": null,
            "email_verified_at": "2020-09-22T13:23:59.000000Z",
            "last_seen_at": null,
            "deleted_at": null,
            "created_at": "2020-09-22T13:23:59.000000Z",
            "updated_at": "2020-09-22T13:23:59.000000Z",
            "role": "Verified Freelancer",
            "rating": null,
            "isActive": false,
            "lastSeen": null,
            "pivot": {
                "business_id": 1,
                "user_id": 16,
                "role_id": 1,
                "apn_token": null,
                "fcm_token": null,
                "email_notifications": true,
                "sms_notifications": true,
                "push_notifications": true,
                "role": {
                    "id": 1,
                    "name": "Verified Freelancer"
                }
            },
            "profile": {
                "id": 16,
                "user_id": 16,
                "image": "https:\/\/randomuser.me\/api\/portraits\/women\/94.jpg",
                "description": "Et ea in reprehenderit sint est.",
                "created_at": "2020-09-22T13:23:59.000000Z",
                "updated_at": "2020-09-22T13:23:59.000000Z"
            }
        },
        {
            "id": 17,
            "first_name": "Bette",
            "last_name": "Fanny",
            "username": "ratke.reid49",
            "email": "hlegros@example.com",
            "phone": "1-446-717-5154 x142",
            "apn_token": null,
            "fcm_token": null,
            "email_verified_at": "2020-09-22T13:23:59.000000Z",
            "last_seen_at": null,
            "deleted_at": null,
            "created_at": "2020-09-22T13:23:59.000000Z",
            "updated_at": "2020-09-22T13:23:59.000000Z",
            "role": "Verified Freelancer",
            "rating": null,
            "isActive": false,
            "lastSeen": null,
            "pivot": {
                "business_id": 1,
                "user_id": 17,
                "role_id": 1,
                "apn_token": null,
                "fcm_token": null,
                "email_notifications": true,
                "sms_notifications": true,
                "push_notifications": true,
                "role": {
                    "id": 1,
                    "name": "Verified Freelancer"
                }
            },
            "profile": {
                "id": 17,
                "user_id": 17,
                "image": "https:\/\/randomuser.me\/api\/portraits\/women\/96.jpg",
                "description": "Ut culpa ullam velit expedita.",
                "created_at": "2020-09-22T13:23:59.000000Z",
                "updated_at": "2020-09-22T13:23:59.000000Z"
            }
        },
        {
            "id": 18,
            "first_name": "Arturo",
            "last_name": "Anabel",
            "username": "sjacobs52",
            "email": "tanner.osinski@example.org",
            "phone": "807-881-4921 x2401",
            "apn_token": null,
            "fcm_token": null,
            "email_verified_at": "2020-09-22T13:23:59.000000Z",
            "last_seen_at": null,
            "deleted_at": null,
            "created_at": "2020-09-22T13:23:59.000000Z",
            "updated_at": "2020-09-22T13:23:59.000000Z",
            "role": "Verified Freelancer",
            "rating": null,
            "isActive": false,
            "lastSeen": null,
            "pivot": {
                "business_id": 1,
                "user_id": 18,
                "role_id": 1,
                "apn_token": null,
                "fcm_token": null,
                "email_notifications": true,
                "sms_notifications": true,
                "push_notifications": true,
                "role": {
                    "id": 1,
                    "name": "Verified Freelancer"
                }
            },
            "profile": {
                "id": 18,
                "user_id": 18,
                "image": "https:\/\/randomuser.me\/api\/portraits\/men\/97.jpg",
                "description": "Voluptas ipsum dolorem dolorum totam.",
                "created_at": "2020-09-22T13:23:59.000000Z",
                "updated_at": "2020-09-22T13:23:59.000000Z"
            }
        },
        {
            "id": 19,
            "first_name": "Sigmund",
            "last_name": "Cierra",
            "username": "barton.alejandrin46",
            "email": "dach.jakayla@example.com",
            "phone": "791.527.1213 x151",
            "apn_token": null,
            "fcm_token": null,
            "email_verified_at": "2020-09-22T13:23:59.000000Z",
            "last_seen_at": null,
            "deleted_at": null,
            "created_at": "2020-09-22T13:23:59.000000Z",
            "updated_at": "2020-09-22T13:23:59.000000Z",
            "role": "Verified Freelancer",
            "rating": null,
            "isActive": false,
            "lastSeen": null,
            "pivot": {
                "business_id": 1,
                "user_id": 19,
                "role_id": 1,
                "apn_token": null,
                "fcm_token": null,
                "email_notifications": true,
                "sms_notifications": true,
                "push_notifications": true,
                "role": {
                    "id": 1,
                    "name": "Verified Freelancer"
                }
            },
            "profile": {
                "id": 19,
                "user_id": 19,
                "image": "https:\/\/randomuser.me\/api\/portraits\/women\/91.jpg",
                "description": "Rerum totam officia est aut doloribus occaecati quo.",
                "created_at": "2020-09-22T13:23:59.000000Z",
                "updated_at": "2020-09-22T13:23:59.000000Z"
            }
        },
        {
            "id": 20,
            "first_name": "Bo",
            "last_name": "Marcelo",
            "username": "kulas.schuyler83",
            "email": "nreynolds@example.net",
            "phone": "(486) 483-3068 x9531",
            "apn_token": null,
            "fcm_token": null,
            "email_verified_at": "2020-09-22T13:23:59.000000Z",
            "last_seen_at": null,
            "deleted_at": null,
            "created_at": "2020-09-22T13:23:59.000000Z",
            "updated_at": "2020-09-22T13:23:59.000000Z",
            "role": "Verified Freelancer",
            "rating": null,
            "isActive": false,
            "lastSeen": null,
            "pivot": {
                "business_id": 1,
                "user_id": 20,
                "role_id": 1,
                "apn_token": null,
                "fcm_token": null,
                "email_notifications": true,
                "sms_notifications": true,
                "push_notifications": true,
                "role": {
                    "id": 1,
                    "name": "Verified Freelancer"
                }
            },
            "profile": {
                "id": 20,
                "user_id": 20,
                "image": "https:\/\/randomuser.me\/api\/portraits\/men\/93.jpg",
                "description": "Voluptatem consequuntur voluptatum quo est quasi.",
                "created_at": "2020-09-22T13:23:59.000000Z",
                "updated_at": "2020-09-22T13:23:59.000000Z"
            }
        },
        {
            "id": 21,
            "first_name": "Rashawn",
            "last_name": "Carson",
            "username": "ucorwin91",
            "email": "evangeline.vandervort@example.net",
            "phone": "+1-534-437-4879",
            "apn_token": null,
            "fcm_token": null,
            "email_verified_at": "2020-09-22T13:23:59.000000Z",
            "last_seen_at": null,
            "deleted_at": null,
            "created_at": "2020-09-22T13:23:59.000000Z",
            "updated_at": "2020-09-22T13:23:59.000000Z",
            "role": "Verified Freelancer",
            "rating": null,
            "isActive": false,
            "lastSeen": null,
            "pivot": {
                "business_id": 1,
                "user_id": 21,
                "role_id": 1,
                "apn_token": null,
                "fcm_token": null,
                "email_notifications": true,
                "sms_notifications": true,
                "push_notifications": true,
                "role": {
                    "id": 1,
                    "name": "Verified Freelancer"
                }
            },
            "profile": {
                "id": 21,
                "user_id": 21,
                "image": "https:\/\/randomuser.me\/api\/portraits\/women\/94.jpg",
                "description": "Dolorem nesciunt mollitia repellat sed nobis quis et enim.",
                "created_at": "2020-09-22T13:23:59.000000Z",
                "updated_at": "2020-09-22T13:23:59.000000Z"
            }
        },
        {
            "id": 22,
            "first_name": "Justyn",
            "last_name": "Alessia",
            "username": "bartell.lyric15",
            "email": "jessyca.reilly@example.com",
            "phone": "+18586393907",
            "apn_token": null,
            "fcm_token": null,
            "email_verified_at": "2020-09-22T13:23:59.000000Z",
            "last_seen_at": null,
            "deleted_at": null,
            "created_at": "2020-09-22T13:23:59.000000Z",
            "updated_at": "2020-09-22T13:23:59.000000Z",
            "role": "Verified Freelancer",
            "rating": null,
            "isActive": false,
            "lastSeen": null,
            "pivot": {
                "business_id": 1,
                "user_id": 22,
                "role_id": 1,
                "apn_token": null,
                "fcm_token": null,
                "email_notifications": true,
                "sms_notifications": true,
                "push_notifications": true,
                "role": {
                    "id": 1,
                    "name": "Verified Freelancer"
                }
            },
            "profile": {
                "id": 22,
                "user_id": 22,
                "image": "https:\/\/randomuser.me\/api\/portraits\/women\/98.jpg",
                "description": "Ex incidunt aut quasi occaecati.",
                "created_at": "2020-09-22T13:23:59.000000Z",
                "updated_at": "2020-09-22T13:23:59.000000Z"
            }
        },
        {
            "id": 23,
            "first_name": "Jaylon",
            "last_name": "Reilly",
            "username": "ross.halvorson1",
            "email": "trace55@example.org",
            "phone": "+1-210-842-1516",
            "apn_token": null,
            "fcm_token": null,
            "email_verified_at": "2020-09-22T13:23:59.000000Z",
            "last_seen_at": null,
            "deleted_at": null,
            "created_at": "2020-09-22T13:23:59.000000Z",
            "updated_at": "2020-09-22T13:23:59.000000Z",
            "role": "Verified Freelancer",
            "rating": null,
            "isActive": false,
            "lastSeen": null,
            "pivot": {
                "business_id": 1,
                "user_id": 23,
                "role_id": 1,
                "apn_token": null,
                "fcm_token": null,
                "email_notifications": true,
                "sms_notifications": true,
                "push_notifications": true,
                "role": {
                    "id": 1,
                    "name": "Verified Freelancer"
                }
            },
            "profile": {
                "id": 23,
                "user_id": 23,
                "image": "https:\/\/randomuser.me\/api\/portraits\/men\/91.jpg",
                "description": "Aperiam ea numquam quia incidunt aut explicabo.",
                "created_at": "2020-09-22T13:23:59.000000Z",
                "updated_at": "2020-09-22T13:23:59.000000Z"
            }
        },
        {
            "id": 24,
            "first_name": "Gertrude",
            "last_name": "Camila",
            "username": "kutch.dallas79",
            "email": "smith.ludwig@example.com",
            "phone": "1-443-374-1118 x14444",
            "apn_token": null,
            "fcm_token": null,
            "email_verified_at": "2020-09-22T13:23:59.000000Z",
            "last_seen_at": null,
            "deleted_at": null,
            "created_at": "2020-09-22T13:23:59.000000Z",
            "updated_at": "2020-09-22T13:23:59.000000Z",
            "role": "Verified Freelancer",
            "rating": null,
            "isActive": false,
            "lastSeen": null,
            "pivot": {
                "business_id": 1,
                "user_id": 24,
                "role_id": 1,
                "apn_token": null,
                "fcm_token": null,
                "email_notifications": true,
                "sms_notifications": true,
                "push_notifications": true,
                "role": {
                    "id": 1,
                    "name": "Verified Freelancer"
                }
            },
            "profile": {
                "id": 24,
                "user_id": 24,
                "image": "https:\/\/randomuser.me\/api\/portraits\/men\/92.jpg",
                "description": "Suscipit modi velit et ipsa corporis.",
                "created_at": "2020-09-22T13:23:59.000000Z",
                "updated_at": "2020-09-22T13:23:59.000000Z"
            }
        },
        {
            "id": 25,
            "first_name": "Giovanni",
            "last_name": "Irwin",
            "username": "blanda.rashad88",
            "email": "pkrajcik@example.com",
            "phone": "271-730-9580 x452",
            "apn_token": null,
            "fcm_token": null,
            "email_verified_at": "2020-09-22T13:23:59.000000Z",
            "last_seen_at": null,
            "deleted_at": null,
            "created_at": "2020-09-22T13:23:59.000000Z",
            "updated_at": "2020-09-22T13:23:59.000000Z",
            "role": "Pending Freelancer",
            "rating": null,
            "isActive": false,
            "lastSeen": null,
            "pivot": {
                "business_id": 1,
                "user_id": 25,
                "role_id": 2,
                "apn_token": null,
                "fcm_token": null,
                "email_notifications": true,
                "sms_notifications": true,
                "push_notifications": true,
                "role": {
                    "id": 2,
                    "name": "Pending Freelancer"
                }
            },
            "profile": {
                "id": 25,
                "user_id": 25,
                "image": "https:\/\/randomuser.me\/api\/portraits\/women\/97.jpg",
                "description": "Quae cupiditate et quis dolores quasi illo.",
                "created_at": "2020-09-22T13:23:59.000000Z",
                "updated_at": "2020-09-22T13:23:59.000000Z"
            }
        },
        {
            "id": 26,
            "first_name": "Charlene",
            "last_name": "Jessie",
            "username": "wbatz86",
            "email": "jerrell.bayer@example.org",
            "phone": "1-381-669-8157 x57143",
            "apn_token": null,
            "fcm_token": null,
            "email_verified_at": "2020-09-22T13:23:59.000000Z",
            "last_seen_at": null,
            "deleted_at": null,
            "created_at": "2020-09-22T13:23:59.000000Z",
            "updated_at": "2020-09-22T13:23:59.000000Z",
            "role": "Pending Freelancer",
            "rating": null,
            "isActive": false,
            "lastSeen": null,
            "pivot": {
                "business_id": 1,
                "user_id": 26,
                "role_id": 2,
                "apn_token": null,
                "fcm_token": null,
                "email_notifications": true,
                "sms_notifications": true,
                "push_notifications": true,
                "role": {
                    "id": 2,
                    "name": "Pending Freelancer"
                }
            },
            "profile": {
                "id": 26,
                "user_id": 26,
                "image": "https:\/\/randomuser.me\/api\/portraits\/women\/98.jpg",
                "description": "Numquam sit distinctio sed voluptatibus possimus.",
                "created_at": "2020-09-22T13:23:59.000000Z",
                "updated_at": "2020-09-22T13:23:59.000000Z"
            }
        },
        {
            "id": 27,
            "first_name": "Leta",
            "last_name": "Geovany",
            "username": "destinee.emard13",
            "email": "ruthe97@example.com",
            "phone": "(596) 438-6092",
            "apn_token": null,
            "fcm_token": null,
            "email_verified_at": "2020-09-22T13:23:59.000000Z",
            "last_seen_at": null,
            "deleted_at": null,
            "created_at": "2020-09-22T13:23:59.000000Z",
            "updated_at": "2020-09-22T13:23:59.000000Z",
            "role": "Pending Freelancer",
            "rating": null,
            "isActive": false,
            "lastSeen": null,
            "pivot": {
                "business_id": 1,
                "user_id": 27,
                "role_id": 2,
                "apn_token": null,
                "fcm_token": null,
                "email_notifications": true,
                "sms_notifications": true,
                "push_notifications": true,
                "role": {
                    "id": 2,
                    "name": "Pending Freelancer"
                }
            },
            "profile": {
                "id": 27,
                "user_id": 27,
                "image": "https:\/\/randomuser.me\/api\/portraits\/men\/96.jpg",
                "description": "Tempore soluta ut quis vitae et sit.",
                "created_at": "2020-09-22T13:23:59.000000Z",
                "updated_at": "2020-09-22T13:23:59.000000Z"
            }
        },
        {
            "id": 28,
            "first_name": "Antoinette",
            "last_name": "Liliana",
            "username": "quigley.carmelo22",
            "email": "ladarius.hagenes@example.net",
            "phone": "1-475-595-6968 x625",
            "apn_token": null,
            "fcm_token": null,
            "email_verified_at": "2020-09-22T13:23:59.000000Z",
            "last_seen_at": null,
            "deleted_at": null,
            "created_at": "2020-09-22T13:23:59.000000Z",
            "updated_at": "2020-09-22T13:23:59.000000Z",
            "role": "Pending Freelancer",
            "rating": null,
            "isActive": false,
            "lastSeen": null,
            "pivot": {
                "business_id": 1,
                "user_id": 28,
                "role_id": 2,
                "apn_token": null,
                "fcm_token": null,
                "email_notifications": true,
                "sms_notifications": true,
                "push_notifications": true,
                "role": {
                    "id": 2,
                    "name": "Pending Freelancer"
                }
            },
            "profile": {
                "id": 28,
                "user_id": 28,
                "image": "https:\/\/randomuser.me\/api\/portraits\/men\/92.jpg",
                "description": "Saepe enim eum tempora quos ullam.",
                "created_at": "2020-09-22T13:23:59.000000Z",
                "updated_at": "2020-09-22T13:23:59.000000Z"
            }
        },
        {
            "id": 29,
            "first_name": "Kacey",
            "last_name": "Earline",
            "username": "cora3558",
            "email": "fnader@example.org",
            "phone": "914-501-3561 x652",
            "apn_token": null,
            "fcm_token": null,
            "email_verified_at": "2020-09-22T13:23:59.000000Z",
            "last_seen_at": null,
            "deleted_at": null,
            "created_at": "2020-09-22T13:23:59.000000Z",
            "updated_at": "2020-09-22T13:23:59.000000Z",
            "role": "Pending Freelancer",
            "rating": null,
            "isActive": false,
            "lastSeen": null,
            "pivot": {
                "business_id": 1,
                "user_id": 29,
                "role_id": 2,
                "apn_token": null,
                "fcm_token": null,
                "email_notifications": true,
                "sms_notifications": true,
                "push_notifications": true,
                "role": {
                    "id": 2,
                    "name": "Pending Freelancer"
                }
            },
            "profile": {
                "id": 29,
                "user_id": 29,
                "image": "https:\/\/randomuser.me\/api\/portraits\/women\/94.jpg",
                "description": "Velit non natus rem nemo et consectetur laudantium saepe.",
                "created_at": "2020-09-22T13:23:59.000000Z",
                "updated_at": "2020-09-22T13:23:59.000000Z"
            }
        },
        {
            "id": 30,
            "first_name": "Luigi",
            "last_name": "Ivy",
            "username": "jettie9787",
            "email": "phoebe15@example.org",
            "phone": "830.902.3027 x8518",
            "apn_token": null,
            "fcm_token": null,
            "email_verified_at": "2020-09-22T13:23:59.000000Z",
            "last_seen_at": null,
            "deleted_at": null,
            "created_at": "2020-09-22T13:23:59.000000Z",
            "updated_at": "2020-09-22T13:23:59.000000Z",
            "role": "Pending Freelancer",
            "rating": null,
            "isActive": false,
            "lastSeen": null,
            "pivot": {
                "business_id": 1,
                "user_id": 30,
                "role_id": 2,
                "apn_token": null,
                "fcm_token": null,
                "email_notifications": true,
                "sms_notifications": true,
                "push_notifications": true,
                "role": {
                    "id": 2,
                    "name": "Pending Freelancer"
                }
            },
            "profile": {
                "id": 30,
                "user_id": 30,
                "image": "https:\/\/randomuser.me\/api\/portraits\/men\/97.jpg",
                "description": "Ut ut voluptas dolores dolorem.",
                "created_at": "2020-09-22T13:23:59.000000Z",
                "updated_at": "2020-09-22T13:23:59.000000Z"
            }
        },
        {
            "id": 31,
            "first_name": "Christiana",
            "last_name": "Raul",
            "username": "ustanton48",
            "email": "alessandro.sauer@example.org",
            "phone": "+1.412.818.7891",
            "apn_token": null,
            "fcm_token": null,
            "email_verified_at": "2020-09-22T13:23:59.000000Z",
            "last_seen_at": null,
            "deleted_at": null,
            "created_at": "2020-09-22T13:23:59.000000Z",
            "updated_at": "2020-09-22T13:23:59.000000Z",
            "role": "Pending Freelancer",
            "rating": null,
            "isActive": false,
            "lastSeen": null,
            "pivot": {
                "business_id": 1,
                "user_id": 31,
                "role_id": 2,
                "apn_token": null,
                "fcm_token": null,
                "email_notifications": true,
                "sms_notifications": true,
                "push_notifications": true,
                "role": {
                    "id": 2,
                    "name": "Pending Freelancer"
                }
            },
            "profile": {
                "id": 31,
                "user_id": 31,
                "image": "https:\/\/randomuser.me\/api\/portraits\/women\/98.jpg",
                "description": "Hic sit aut harum.",
                "created_at": "2020-09-22T13:23:59.000000Z",
                "updated_at": "2020-09-22T13:23:59.000000Z"
            }
        },
        {
            "id": 32,
            "first_name": "Julianne",
            "last_name": "Immanuel",
            "username": "eldred.jast25",
            "email": "uspinka@example.org",
            "phone": "970.280.6369",
            "apn_token": null,
            "fcm_token": null,
            "email_verified_at": "2020-09-22T13:23:59.000000Z",
            "last_seen_at": null,
            "deleted_at": null,
            "created_at": "2020-09-22T13:23:59.000000Z",
            "updated_at": "2020-09-22T13:23:59.000000Z",
            "role": "Pending Freelancer",
            "rating": null,
            "isActive": false,
            "lastSeen": null,
            "pivot": {
                "business_id": 1,
                "user_id": 32,
                "role_id": 2,
                "apn_token": null,
                "fcm_token": null,
                "email_notifications": true,
                "sms_notifications": true,
                "push_notifications": true,
                "role": {
                    "id": 2,
                    "name": "Pending Freelancer"
                }
            },
            "profile": {
                "id": 32,
                "user_id": 32,
                "image": "https:\/\/randomuser.me\/api\/portraits\/women\/97.jpg",
                "description": "Dolorem odio eius voluptates cupiditate quia.",
                "created_at": "2020-09-22T13:23:59.000000Z",
                "updated_at": "2020-09-22T13:23:59.000000Z"
            }
        },
        {
            "id": 33,
            "first_name": "Bradley",
            "last_name": "Buster",
            "username": "garret7984",
            "email": "lind.shirley@example.org",
            "phone": "+1-602-431-1277",
            "apn_token": null,
            "fcm_token": null,
            "email_verified_at": "2020-09-22T13:23:59.000000Z",
            "last_seen_at": null,
            "deleted_at": null,
            "created_at": "2020-09-22T13:23:59.000000Z",
            "updated_at": "2020-09-22T13:23:59.000000Z",
            "role": "Pending Freelancer",
            "rating": null,
            "isActive": false,
            "lastSeen": null,
            "pivot": {
                "business_id": 1,
                "user_id": 33,
                "role_id": 2,
                "apn_token": null,
                "fcm_token": null,
                "email_notifications": true,
                "sms_notifications": true,
                "push_notifications": true,
                "role": {
                    "id": 2,
                    "name": "Pending Freelancer"
                }
            },
            "profile": {
                "id": 33,
                "user_id": 33,
                "image": "https:\/\/randomuser.me\/api\/portraits\/women\/95.jpg",
                "description": "Qui doloribus distinctio tempore et ab ad illo.",
                "created_at": "2020-09-22T13:23:59.000000Z",
                "updated_at": "2020-09-22T13:23:59.000000Z"
            }
        },
        {
            "id": 34,
            "first_name": "Rhianna",
            "last_name": "Erick",
            "username": "yazmin.lowe14",
            "email": "fwisoky@example.org",
            "phone": "+12752684836",
            "apn_token": null,
            "fcm_token": null,
            "email_verified_at": "2020-09-22T13:23:59.000000Z",
            "last_seen_at": null,
            "deleted_at": null,
            "created_at": "2020-09-22T13:23:59.000000Z",
            "updated_at": "2020-09-22T13:23:59.000000Z",
            "role": "Pending Freelancer",
            "rating": null,
            "isActive": false,
            "lastSeen": null,
            "pivot": {
                "business_id": 1,
                "user_id": 34,
                "role_id": 2,
                "apn_token": null,
                "fcm_token": null,
                "email_notifications": true,
                "sms_notifications": true,
                "push_notifications": true,
                "role": {
                    "id": 2,
                    "name": "Pending Freelancer"
                }
            },
            "profile": {
                "id": 34,
                "user_id": 34,
                "image": "https:\/\/randomuser.me\/api\/portraits\/men\/91.jpg",
                "description": "Deserunt non exercitationem nihil qui velit.",
                "created_at": "2020-09-22T13:23:59.000000Z",
                "updated_at": "2020-09-22T13:23:59.000000Z"
            }
        }
    ]
}

```



<a name="show-user"></a>
## Show User

View a single user.
### Endpoint
|Method|URI|Authentication|
|:-|:-|:-|
|`GET`|`/business/{unique_id}/user/{id}`|`true`|



> {success} Example Success Response
Code `200`

Content

```json
{
    "success": true,
    "message": "Show user",
    "data": {
        "id": 2,
        "first_name": "Angelina",
        "last_name": "Jennie",
        "username": "worker_one",
        "email": "worker_one@mail.com",
        "phone": "589-200-0246",
        "apn_token": null,
        "fcm_token": null,
        "email_verified_at": "2020-09-22T13:23:54.000000Z",
        "last_seen_at": null,
        "deleted_at": null,
        "created_at": "2020-09-22T13:23:54.000000Z",
        "updated_at": "2020-09-22T13:23:54.000000Z",
        "role": "Verified Freelancer",
        "rating": 5,
        "isActive": false,
        "lastSeen": null,
        "pivot": {
            "business_id": 1,
            "user_id": 2,
            "role_id": 1,
            "apn_token": null,
            "fcm_token": null,
            "email_notifications": true,
            "sms_notifications": true,
            "push_notifications": true,
            "role": {
                "id": 1,
                "name": "Verified Freelancer"
            }
        },
        "profile": {
            "id": 2,
            "user_id": 2,
            "image": "https:\/\/randomuser.me\/api\/portraits\/men\/92.jpg",
            "description": "Molestiae consequuntur dolores omnis possimus.",
            "created_at": "2020-09-22T13:23:54.000000Z",
            "updated_at": "2020-09-22T13:23:54.000000Z"
        },
        "marketplace_proposals": [
            {
                "id": 1,
                "marketplace_id": 2,
                "user_id": 2,
                "status_id": 1,
                "rating": null,
                "review": null,
                "arrived_at": null,
                "completed_at": null,
                "created_at": "2020-09-22T13:24:00.000000Z",
                "updated_at": "2020-09-22T13:24:00.000000Z",
                "status": "Pending",
                "marketplace_job": {
                    "id": 2,
                    "business_id": 1,
                    "customer_id": 1,
                    "category_id": 1,
                    "client_name": "Lois Deckow",
                    "price": 12,
                    "description": "Aliquid dolorum quia non voluptatum. Fugiat et alias autem delectus eum.",
                    "status_id": 2,
                    "intensity_id": 3,
                    "complete_before": "2020-09-23 00:00:00",
                    "views": 0,
                    "image_one": "https:\/\/www.simplemost.com\/wp-content\/uploads\/2017\/01\/8320434990_4c84ea6e62_o-750x500.jpg",
                    "image_two": null,
                    "image_three": null,
                    "created_at": "2020-09-22T13:24:00.000000Z",
                    "updated_at": "2020-09-22T13:24:00.000000Z",
                    "deleted_at": null,
                    "status": "Pending Approval",
                    "intensity": "Hard (more than 4 hours)",
                    "job_status": {
                        "id": 2,
                        "name": "Pending Approval"
                    },
                    "job_intensity": {
                        "id": 3,
                        "name": "Hard (more than 4 hours)"
                    }
                },
                "proposal_status": {
                    "id": 1,
                    "name": "Pending"
                }
            },
            {
                "id": 2,
                "marketplace_id": 3,
                "user_id": 2,
                "status_id": 2,
                "rating": null,
                "review": null,
                "arrived_at": null,
                "completed_at": null,
                "created_at": "2020-09-22T13:24:01.000000Z",
                "updated_at": "2020-09-22T13:24:01.000000Z",
                "status": "Approved",
                "marketplace_job": {
                    "id": 3,
                    "business_id": 1,
                    "customer_id": 1,
                    "category_id": 1,
                    "client_name": "Mr. Candido Streich",
                    "price": 16,
                    "description": "Deserunt in exercitationem aut. Mollitia in aut qui eos tenetur et architecto accusantium.",
                    "status_id": 3,
                    "intensity_id": 3,
                    "complete_before": "2020-09-23 00:00:00",
                    "views": 0,
                    "image_one": "https:\/\/thenewswheel.com\/wp-content\/uploads\/2018\/04\/junk-yards-pay-most-for-cars-760x507.jpg",
                    "image_two": null,
                    "image_three": null,
                    "created_at": "2020-09-22T13:24:01.000000Z",
                    "updated_at": "2020-09-22T13:24:01.000000Z",
                    "deleted_at": null,
                    "status": "Approved",
                    "intensity": "Hard (more than 4 hours)",
                    "job_status": {
                        "id": 3,
                        "name": "Approved"
                    },
                    "job_intensity": {
                        "id": 3,
                        "name": "Hard (more than 4 hours)"
                    }
                },
                "proposal_status": {
                    "id": 2,
                    "name": "Approved"
                }
            },
            {
                "id": 3,
                "marketplace_id": 4,
                "user_id": 2,
                "status_id": 2,
                "rating": null,
                "review": null,
                "arrived_at": "2020-09-22 00:00:00",
                "completed_at": null,
                "created_at": "2020-09-22T13:24:01.000000Z",
                "updated_at": "2020-09-22T13:24:01.000000Z",
                "status": "Approved",
                "marketplace_job": {
                    "id": 4,
                    "business_id": 1,
                    "customer_id": 1,
                    "category_id": 1,
                    "client_name": "Helena Terry",
                    "price": 38,
                    "description": "Numquam quos in id consequuntur. Autem quam neque quos velit qui est fuga.",
                    "status_id": 4,
                    "intensity_id": 3,
                    "complete_before": "2020-09-23 00:00:00",
                    "views": 0,
                    "image_one": "https:\/\/thenewswheel.com\/wp-content\/uploads\/2018\/04\/junk-yards-pay-most-for-cars-760x507.jpg",
                    "image_two": null,
                    "image_three": null,
                    "created_at": "2020-09-22T13:24:01.000000Z",
                    "updated_at": "2020-09-22T13:24:01.000000Z",
                    "deleted_at": null,
                    "status": "In Progress",
                    "intensity": "Hard (more than 4 hours)",
                    "job_status": {
                        "id": 4,
                        "name": "In Progress"
                    },
                    "job_intensity": {
                        "id": 3,
                        "name": "Hard (more than 4 hours)"
                    }
                },
                "proposal_status": {
                    "id": 2,
                    "name": "Approved"
                }
            },
            {
                "id": 4,
                "marketplace_id": 5,
                "user_id": 2,
                "status_id": 2,
                "rating": "5",
                "review": "Good job!",
                "arrived_at": "2020-09-22 00:00:00",
                "completed_at": "2020-09-21 23:00:00",
                "created_at": "2020-09-22T13:24:01.000000Z",
                "updated_at": "2020-09-22T13:24:01.000000Z",
                "status": "Approved",
                "marketplace_job": {
                    "id": 5,
                    "business_id": 1,
                    "customer_id": 1,
                    "category_id": 1,
                    "client_name": "Rusty Weimann",
                    "price": 19,
                    "description": "Culpa optio rerum vero quis et qui. Quia et provident asperiores voluptate.",
                    "status_id": 5,
                    "intensity_id": 3,
                    "complete_before": "2020-09-23 00:00:00",
                    "views": 0,
                    "image_one": "https:\/\/www.simplemost.com\/wp-content\/uploads\/2017\/01\/8320434990_4c84ea6e62_o-750x500.jpg",
                    "image_two": null,
                    "image_three": null,
                    "created_at": "2020-09-22T13:24:01.000000Z",
                    "updated_at": "2020-09-22T13:24:01.000000Z",
                    "deleted_at": null,
                    "status": "Complete",
                    "intensity": "Hard (more than 4 hours)",
                    "job_status": {
                        "id": 5,
                        "name": "Complete"
                    },
                    "job_intensity": {
                        "id": 3,
                        "name": "Hard (more than 4 hours)"
                    }
                },
                "proposal_status": {
                    "id": 2,
                    "name": "Approved"
                }
            }
        ],
        "payouts": [],
        "payments": [],
        "marketplace_jobs": []
    }
}

```



<a name="update-role"></a>
## Update Role

Update a user&#039;s role within a business.
### Endpoint
|Method|URI|Authentication|
|:-|:-|:-|
|`PATCH`|`/business/{unique_id}/user/{id}`|`true`|



> {success} Example Success Response
Code `200`

Content

```json
{
    "success": true,
    "message": "This users role has been updated.",
    "data": null
}

```



<a name="remove-user"></a>
## Remove User

This removes a user from your business marketplace. This action is irreverisble
### Endpoint
|Method|URI|Authentication|
|:-|:-|:-|
|`DELETE`|`/business/{unique_id}/user/{id}`|`true`|



> {success} Example Success Response
Code `200`

Content

```json
{
    "success": true,
    "message": "This user has been removed from your business.",
    "data": null
}

```


